...
coverage: test/Manifest.toml ## Run the test suite with coverage (requires `LocalCoverage` in base env)
	$(JULIA) --project=test -e 'using LocalCoverage; report = generate_coverage("Bijections"; run_test = true, folder_list = ["src", "ext"]); show(report)'
...
codestyle: test/Manifest.toml ## Apply the codestyle to the entire project
	$(JULIA) --project=test -e 'using JuliaFormatter; format(["src", "docs", "test", "ext"], verbose=true)'
